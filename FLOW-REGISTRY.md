...
| Understand current Brokerage position | Implemented. | Home → scan value, annual distributions, authentic history, then the four largest holdings; search or open an asset when needed. | Empty; private loaded; fewer than two snapshots; search no-match; allocation warning. | Home structure, Acadia boundary and calculation tests. |
...
| Export private Brokerage data | Implemented. | Export → owner downloads a JSON copy of their Brokerage holdings, quotes and snapshots. | Signed-in owner export. | Client export contract; RLS database acceptance required after migration. |
