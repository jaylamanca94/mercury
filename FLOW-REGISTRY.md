...
| Understand current Brokerage position | Implemented. | Home → scan value, annual distributions, authentic performance, then search, filter, sort, or open the first four matching investments. | Empty; private loaded; fewer than two snapshots; search/filter no-match; allocation warning. | Home structure, Acadia boundary and calculation tests. |
