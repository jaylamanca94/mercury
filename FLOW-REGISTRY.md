...
| Understand current position | Implemented; Home balance simplification 0.2.16. | Home → exact investment balance → YTD/1Y/All graph and value change → optional Values and calculations → Brokerage, Crypto, Retirement or Property → scoped Portfolio. | Zero/one/many observations; incomplete range; same-day current endpoint; missing valuation; zero baseline; stale saved prices; missing property; keyboard periods; responsive/enlarged text. | Calculation/controller checks and isolated synthetic browser evidence in [Home balance validation](automation/home-balance/2026-09-23/review.md). |
