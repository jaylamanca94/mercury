...
| Understand current position | Implemented; simplified 2026-09-05. | Home → net worth and curved recorded history → grouped all-time/day changes and annual estimates, alongside allocation → top asset/property card, View all → Portfolio, or Add asset. | Empty; private loaded; missing valuations/metrics/property data; zero/one/two/sparse daily dates; long recorded history; long names; desktop/tablet/phone; light/dark; keyboard range and card actions. | 129 passing tests, including compact/full-history transition coverage; isolated browser checks at 1920/1440/1024/768/390/320px; see `design-qa.md`. |
...
| Review current trajectory | Implemented; Figma Plan refinement 2026-09-14, 0.1.0. | Plan → source-linked cash flows → set current age → edit amounts/stop-investing/retirement → live graph → select horizon and year → Save Plan; current asset groups link to Portfolio. | Linked/null versus zero overrides; missing cash-flow/rate/valuation data; entered age; monthly contributions and retirement withdrawals; depletion; selected-year synchronisation; failed/uncertain/conflicting saves; pending-write lock; guarded navigation; account replacement; reset to source amounts. | 233 automated checks; authenticated disposable-account persistence, revision and constraint checks with verified cleanup; responsive 320/390/768/1024/1512px and 200% text, keyboard year control, save/cancel/conflict and asset-group navigation. See Plan refinement receipt. |
