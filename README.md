...
Version **0.2.17** — Home includes property in its single net-worth balance, graph and selected-period change: investments plus property value minus mortgage debt. Daily snapshots now preserve property equity alongside investments. Older investment-only records remain intact and are excluded from the net-worth graph rather than guessed. See [property-inclusive Home validation](automation/home-net-worth/2026-09-23/review.md).

Previous **0.2.14** — Portfolio filters, sort controls and menu items retain keyboard focus in forced-colour palettes through Acadia 0.4.10. Pressed filters remain distinguishable from the focused control. Automatic quotes preserve provider timestamps and reject missing or ambiguous dates instead of implying freshness. Authentication requests recover from stalls with visible retry feedback. Profile menus remain visible across desktop, tablet and phone. All 13 shared runtime assets match the released Acadia 0.4.10 source. Home defaults to a value-weighted market-movement chart, with saved daily value history available separately. Schema rebuild, synthetic product-data restore and two-user database isolation have local/live evidence; remaining pre-1.0 acceptance gates are below.

Mercury is a private personal-finance workspace. Home shows current net worth (investments plus property equity) and its observed history. YTD, 1Y and All affect the graph and both change figures together; partial history names the actual starting date. Value changes include deposits, withdrawals and edits, and are not labelled investment returns. Saved quote dates are visible; the display does not imply live prices. The component breakdown, investment-only day/lifetime changes and annual illustrations sit in Values and calculations. Brokerage, Crypto, Retirement and Property groups open scoped Portfolio destinations. Allocation remains in Portfolio. Portfolio leads with the selected investment value and matching Cards or Table views. Shared native filter buttons select All investments, Brokerage, Retirement or Crypto on every screen, with a visible checkmark on the current group. Its precise value, count and percentage share stay together; retirement crypto belongs only to Retirement. An adaptive Acadia grid gives cards room on tablets and uses more columns on wider screens. Investment assets automatically appear in descending current holding value in matching Cards or Table views, with unvalued assets last. There are no investment search or manual-sort controls. Group selection affects records only; incomplete valuations withhold affected totals and every share. Allocation expands below investments, while property equity and weekly-equivalent Recurring totals sit beside their records. Add an optional purchase price in the property editor to see current market value minus purchase price, in dollars and percent. This value change excludes debt, costs and rental income; equity remains market value minus debt. Group filtering, empty-group recovery and missing-valuation repair remain available across device sizes. Income opens a monthly planning Overview with a balance-led Acadia summary, gross recurring source cards alongside compact annual dividend estimates, and explicit Edit dialogs; `#income/budget` opens monthly category-level spending limits. Planned balance subtracts planned spending and investing from expected income and is never presented as spendable cash. Plan presents one illustrative Base plan with a compact missing-input state, paired investment-value and annual portfolio-income outlooks, and explicit assumption sources. Incomplete investment valuations withhold the outlook; property equity contributes to projected net worth and stays separate from spendable investments.
...
2. Follow [`supabase/README.md`](supabase/README.md) to apply the eight active migrations to a new environment or validate the existing linked history. Archived scripts are historical references, not migrations to replay.
...
- Magic-link sending and sign-out have ten-second deadlines, single-flight controls and context guards. A timeout does not cancel Supabase Auth: mail or an auth event can still arrive. The UI reports an unconfirmed outcome and offers retry; late promise results cannot overwrite retry feedback. Profile keeps its canonical Acadia menu open for pending/error feedback, opens above the phone dock, and escapes navigation-track clipping.
...
- `acadia-card-trend.mjs` — unchanged Acadia curve utility from published `6c3703d680c1f8bed23348148570eb095f4a4b60`, with per-file provenance in `acadia-vendor.json`. Home imports it through its module entry point; stylesheet provenance is tracked separately.
...
- `acadia.css`, `fonts/`, `assets/accordion-*.svg`, `assets/auth-divider.svg`, `assets/icons/house-solid.svg` — unchanged Acadia assets from published Acadia main revision `6c3703d680c1f8bed23348148570eb095f4a4b60` (0.4.10). `acadia-vendor.json` records their origin and hashes; the test suite checks integrity and that active Acadia class names have definitions. Table is included in the single stylesheet. `styles.css` records the phone editor offset, form box-sizing correction, bounded dialog spacing and Income composition adapters.
...
## Complete collection reads — 0.2.6

`collection-read.js` is shared by the browser and snapshot endpoint. Reads request exact counts and stable ordering with an ID tie-breaker, use pages of at most 500 rows, and continue from the actual returned length when the server caps pages below that size. Missing/changing counts, duplicate/missing IDs, malformed responses, no progress, or more than 100,000 records fail closed. Each collection retains a ten-second total deadline across its pages. Results reach calculations only after the whole collection succeeds; required failures retain the existing workspace recovery and optional failures remain local. Income/Property retries use the same path. Quote reads use an account-filtered inner join to holdings, avoiding a growing URL of holding IDs.

This prevents silent row-limit truncation; it does not create a database transaction across requests. Same-count concurrent edits can still represent adjacent moments. Atomic database-side aggregation and a latest-quote view are future options if volume warrants them. No migration or new dependency is needed. Current evidence: `automation/review/2026-09-21/review.md`.

## Confirmed mutations — 0.2.7

Add asset uses a stable insert ID and returned record; an uncertain retry cannot overwrite a saved asset. Holding/quote writes and all four deletion paths use the existing ten-second request deadline. Timeouts retain recovery with explicit unconfirmed copy; late responses cannot enter a changed account. Deletion requires its returned ID or a successful scoped absence read, allowing a retry after a lost response. Source/category/property deletion updates acknowledged local collections without unrelated account reloads. Existing Acadia dialogs, pending locks and focus recovery are retained.

Two disposable authenticated users verified own access and rejection of cross-user reads, updates, deletes, foreign-parent inserts and ownership changes across all eight private tables. Anonymous reads and the account-filtered quote join were also checked. Both users and their records were removed. See `automation/review/2026-09-21/reliability/`. No migration or policy changes were necessary; migration rebuild, real email, scheduler/provider and physical-device acceptance remain separate.

## Database rebuild — 0.2.8

`npm run check:database` starts an isolated PostgreSQL 17 cluster, applies all seven active migrations using the Supabase CLI, compares the reviewed hosted schema contract, checks ownership/constraints and removes the temporary cluster. Requires PostgreSQL 17, Python 3 and Supabase CLI; see `supabase/README.md`. The standard `npm run check` also rejects duplicate/invalid migration versions and verifies archived SQL provenance. This release fixes environment reproducibility without changing production schema, records, application behaviour or Acadia assets. Hosted migration versions match and the push dry run is empty.

## Historical Home market movement — 0.2.9 (superseded by 0.2.16)

Home now defaults to 1M market-price movement for current holdings, reusing the Portfolio model and existing Yahoo daily-history endpoint. `market-history.js` returns every shared date in addition to the aggregate change; the browser draws percentage movement from the starting basket value. Current quantities remain fixed, cash remains constant and property/dividends/past cash flows are excluded. The chart is a current-basket reconstruction, not a personal return series. 1D uses the latest two common observations within the last seven days; 5Y requests the existing maximum history window. Exact available dates remain visible.

Home and Portfolio share an in-memory five-minute history cache with three concurrent reads. No new provider credentials, persistence or migration. Recorded snapshots remain in Chart view → Recorded, with All selected by default. The snapshot writer still uses saved valuations and does not refresh market quotes. Home's new price line therefore does not rewrite or repair historical balances. Million-scale net worth shows two decimals, with precise currency in its title/accessibility label. Evidence and limitations: `automation/home-market/2026-09-22/review.md`.


## Quote date integrity — 0.2.11

Automatic quotes prefer Twelve Data's `last_quote_at` (last quoted minute), then `timestamp` (opening bar); an explicit timezone-bearing ISO datetime is the final supported fallback. Date-only or exchange-local strings cannot establish an instant by themselves. Missing, malformed and future times trigger existing retry/manual-valuation recovery rather than receiving the request time. Cached quotes retain the original provider date. Existing saved records and historical snapshots are not rewritten.
