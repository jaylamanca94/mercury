...
Version **0.2.12** — Keyboard focus remains visible in forced-colour palettes through Acadia 0.4.7. Automatic quotes preserve provider timestamps and reject missing or ambiguous dates instead of implying freshness. Authentication requests recover from stalls with visible retry feedback. Profile menus remain visible across desktop, tablet and phone. All 13 shared runtime assets match the released Acadia 0.4.7 source. Home defaults to a value-weighted market-movement chart, with saved daily value history available separately. Schema rebuild, synthetic product-data restore and two-user database isolation have local/live evidence; remaining pre-1.0 acceptance gates are below.
...
- `acadia-card-trend.mjs` — unchanged Acadia curve utility from published `03e977b8eb02778d015d38766b39164fd7af2235`, with per-file provenance in `acadia-vendor.json`. Home imports it through its module entry point; stylesheet provenance is tracked separately.
...
- `acadia.css`, `fonts/`, `assets/accordion-*.svg`, `assets/auth-divider.svg`, `assets/icons/house-solid.svg` — unchanged Acadia assets from published Acadia main revision `03e977b8eb02778d015d38766b39164fd7af2235` (0.4.7). `acadia-vendor.json` records their origin and hashes; the test suite checks integrity and that active Acadia class names have definitions. Table is included in the single stylesheet. `styles.css` records the phone editor offset, form box-sizing correction, bounded dialog spacing and Income composition adapters.
