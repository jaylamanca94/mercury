...
Version **0.2.11** — Automatic quotes preserve provider timestamps and reject missing or ambiguous dates instead of implying freshness. Authentication requests recover from stalls with visible retry feedback. Profile menus remain visible across desktop, tablet and phone. Acadia 0.4.2 provenance is verified; all 13 shared runtime assets are unchanged. Home defaults to a value-weighted market-movement chart, with saved daily value history available separately. Schema rebuild, synthetic product-data restore and two-user database isolation have local/live evidence; remaining pre-1.0 acceptance gates are below.
...
- `acadia-card-trend.mjs` — unchanged Acadia curve utility from published `c660e2073d78906ea708edfc47e451ba292611be`, with per-file provenance in `acadia-vendor.json`. Home imports it through its module entry point; stylesheet provenance is tracked separately.
...
- `acadia.css`, `fonts/`, `assets/accordion-*.svg`, `assets/auth-divider.svg`, `assets/icons/house-solid.svg` — unchanged Acadia assets from published Acadia main revision `c660e2073d78906ea708edfc47e451ba292611be` (0.4.2). `acadia-vendor.json` records their origin and hashes; the test suite checks integrity and that active Acadia class names have definitions. Table is included in the single stylesheet. `styles.css` records the phone editor offset, form box-sizing correction, bounded dialog spacing and Income composition adapters.
