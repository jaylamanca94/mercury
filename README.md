...
Version **0.2.13** — Desktop navigation focus and selected tabs remain distinguishable in forced-colour palettes through Acadia 0.4.8. Automatic quotes preserve provider timestamps and reject missing or ambiguous dates instead of implying freshness. Authentication requests recover from stalls with visible retry feedback. Profile menus remain visible across desktop, tablet and phone. All 13 shared runtime assets match the released Acadia 0.4.8 source. Home defaults to a value-weighted market-movement chart, with saved daily value history available separately. Schema rebuild, synthetic product-data restore and two-user database isolation have local/live evidence; remaining pre-1.0 acceptance gates are below.
...
- `acadia-card-trend.mjs` — unchanged Acadia curve utility from published `1df9f5889b9e826a9bc17f947b37f214f4a93a66`, with per-file provenance in `acadia-vendor.json`. Home imports it through its module entry point; stylesheet provenance is tracked separately.
...
- `acadia.css`, `fonts/`, `assets/accordion-*.svg`, `assets/auth-divider.svg`, `assets/icons/house-solid.svg` — unchanged Acadia assets from published Acadia main revision `1df9f5889b9e826a9bc17f947b37f214f4a93a66` (0.4.8). `acadia-vendor.json` records their origin and hashes; the test suite checks integrity and that active Acadia class names have definitions. Table is included in the single stylesheet. `styles.css` records the phone editor offset, form box-sizing correction, bounded dialog spacing and Income composition adapters.
