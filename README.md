...
Version **0.2.14** — Portfolio filters, sort controls and menu items retain keyboard focus in forced-colour palettes through Acadia 0.4.10. Pressed filters remain distinguishable from the focused control. Automatic quotes preserve provider timestamps and reject missing or ambiguous dates instead of implying freshness. Authentication requests recover from stalls with visible retry feedback. Profile menus remain visible across desktop, tablet and phone. All 13 shared runtime assets match the released Acadia 0.4.10 source. Home defaults to a value-weighted market-movement chart, with saved daily value history available separately. Schema rebuild, synthetic product-data restore and two-user database isolation have local/live evidence; remaining pre-1.0 acceptance gates are below.
