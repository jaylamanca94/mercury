...
Version **0.2.16** — Home now leads with one exact investment value, an observed balance-history graph, and matching dollar/percentage changes for YTD, 1Y or All. The current endpoint uses the same complete saved valuation as the headline. Calculation details retain net worth, property equity and estimates. See [Home balance validation](automation/home-balance/2026-09-23/review.md).
...
2. Follow [`supabase/README.md`](supabase/README.md) to apply the seven active migrations to a new environment or validate the existing linked history. Archived scripts are historical references, not migrations to replay.
