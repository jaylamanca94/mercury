...
The active `migrations/` directory now has eight unique versions matching the linked hosted ledger. `20260903004833_remote_baseline.sql` bootstraps the existing Mercury schema on an empty Supabase database; the seven later recorded migrations retain their original versions and contents. The baseline replaces a local no-op and incorporates seven previously untracked scripts, preserved byte-for-byte under `archive/pre-baseline/` with SHA-256 provenance.
...
## Net worth snapshot coverage — 0.2.17

`20260924004500_snapshot_property_equity.sql` adds nullable signed `property_equity_cents` and safe-integer checks for equity and combined net worth. Apply this forward migration before the 0.2.17 snapshot writer. Existing rows keep null; never backfill from current properties. The investment column retains its original meaning. The writer reads all account properties, subtracts mortgage debt and writes both components together; a failed/incomplete property read leaves history untouched. Empty property collections record explicit zero. Existing owner-read-only snapshot RLS and grants remain unchanged. Local rebuild/restore checks cover the new column. The forward migration is applied to linked Mercury; fresh hosted structural metadata matches the tested contract and a subsequent push dry run is empty.
...
- `checks/expected-schema.json` is the reviewed hosted product contract captured on 2026-09-22 and updated for the verified 0.2.17 forward migration: eight tables, 88 columns, 75 constraints, 18 indexes, eight policies, six triggers, one helper and 168 role/privilege checks. Function body indentation is ignored; other definitions compare exactly. Supabase-managed infrastructure such as `auth.users` and the optional `public.rls_auto_enable` event helper is excluded.
