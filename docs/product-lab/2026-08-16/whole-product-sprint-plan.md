...
| --- | --- | --- | --- | --- |
| 0. Delivery baseline | Make the existing validated recovery work durable. | Re-check the current checkout, tests, commit chain, and remote state; publish the two local commits if the remote permits. | Clean main; `origin/main` includes the recovery hierarchy and Figma-Home commits. | Completed pending final push |
