...
The all-investment summary stays independent of group/search/sort and Cards/Table. The current holding-value headline remains current. Every required market history must be available; never show a partial aggregate. Unknown share counts (including manual-total investments), missing/one-point/non-overlapping history and provider failures withhold the change with context. Retry is available even when a failed holding is filtered out or Table is selected. No account snapshots, API, database or persistence changes are needed. Home's recorded-history chart is unchanged.
