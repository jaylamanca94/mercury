...
- Home separates the owner's current Net worth from investment performance. Net worth combines every holding with current property equity; the compact line and `Portfolio change · {period}` use authentic portfolio snapshots only. Chronological range tabs lead into start, end, latest plotted portfolio value, and earliest-history context, while the trend remains empty until two snapshots exist. A single metric strip reports prior-close movement, source-backed estimated annual growth, and estimated annual income with their bases. Top assets ranks holdings and property equity together in four compact rows with truthful Brokerage, Retirement, Crypto, or Property classification and links onward to the full Portfolio workspace.
