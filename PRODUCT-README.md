...
## Property-inclusive Home — 2026-09-23

0.2.17 corrects Home's scope to the founder's full net worth: investments plus saved property value minus mortgage debt. The headline, current graph endpoint and period-change baseline all use this scope. Negative equity and negative net worth remain valid; percent change is unavailable for a zero or negative baseline. Missing investment or property data withholds complete net worth rather than showing a partial total.

Daily snapshots retain their investment total and add nullable `property_equity_cents`. A recorded zero means no property equity; null means unknown historical coverage. Only complete snapshots participate in the net-worth graph. Existing investment-only history remains available to investment calculations, but cannot be relabelled as historical net worth or backfilled with today's property value. Until another complete day exists, Home displays current net worth and “Net worth history is building”. Current valuations remain saved values, not live property appraisals or financial returns. No new provider, forecasting assumption or cost is introduced.

The forward database migration preserves prior balances and access controls. Snapshot writes require complete valid property reads, subtract mortgage balances, and reject unsafe totals. See [validation](automation/home-net-worth/2026-09-23/review.md). This supersedes 0.2.16's investment-only Home scope below.

## Home balance simplification — 2026-09-23

Version 0.2.16 follows the founder's supplied Vanguard screenshot as an interaction reference: one exact investment balance, its graph, selected-period dollar/percentage value change, and YTD / 1Y / All. This supersedes the former Home market/recorded toggle, net-worth headline and four summary cards. Secondary values remain in one calculation disclosure; net worth still includes property equity, while the primary balance and all plotted history cover investments only. Portfolio and Plan retain their existing jobs.

Past values come from saved investment snapshots. The current point uses today's holdings and latest saved prices/manual values, exactly matching the headline; it does not write a snapshot. Existing snapshots are not rewritten or backfilled using today's share counts. YTD starts from an observed prior December 31 value, 1Y from its calendar-year boundary, and All from the first record. If a requested boundary has no observation, use the first available record within the range and visibly say “since [actual date]”. Zero baselines have no percentage; a lone observation has no claimed change. Incomplete investment valuations withhold the balance, line and change; unavailable property data does not hide complete investments. Price dates use New York calendar dates, and the source note says saved valuations. This is not a live-price upgrade or a cash-flow-adjusted return calculation.

The screenshot is a private reference: its balances and identifiers are not copied into fixtures or published assets. No new flow, provider, schema, write, licence cost or service is introduced. Removing Home's historical-price fetches reduces unnecessary provider work. See [Home balance validation](automation/home-balance/2026-09-23/review.md).
...
Current: **0.2.17** — Property-inclusive Home above.

Previous: **0.2.14** — Portfolio filters, period controls and sort disclosures retain keyboard focus in forced-colour palettes through unchanged Acadia 0.4.10. No financial, persistence or workflow changes.
...
Previous: **0.2.8** — Migration history is reconciled and the product schema rebuilds from an empty PostgreSQL 17 database with verified hosted parity, ownership, constraints and repeat-run safety. The existing hosted schema/data/history and all user flows remain unchanged. This closes the product-schema rebuild release gate; hosted services, real email, physical-device, provider/scheduler and hosted backup/restore acceptance remain separate. A subsequent synthetic eight-table recovery drill verifies exact product-data restoration and atomic failure handling; see `supabase/RECOVERY.md`. This tooling/evidence-only work retains version 0.2.8. Hosted inspection subsequently confirmed seven completed daily physical backups and PITR off; isolated hosted restoration remains pending explicit approval of its quoted $9.68/month project and full database/user copy.
...
- Home uses only canonical Acadia styles, components, patterns, utilities, icons, responsive navigation, and font assets. Mercury documents bounded page-composition adapters; shared component styling stays canonical.
...
Previous: **0.2.0**. Founder approved structured property geography and automatic area-based appreciation in Plan. Use FHFA's public county ten-year annualised nominal HPI change as a clearly dated assumption, with an optional owner override. Include projected property equity in Plan net worth without treating it as investment income or available cash. Preserve actual market/debt/purchase inputs and legacy location text; missing growth is explicit and never inferred from a neighbouring area. Data coverage, source release and refresh boundaries are documented in README.
...
## Portfolio market-performance summary — 2026-09-16

The founder corrected the earlier recorded-value choice: Portfolio summaries must show **market-price performance of the assets**, excluding deposits and withdrawals. The two percentage captions and period dollar change now use the same current-holdings basket: sum current shares × (last price − first price), with percentage relative to current shares × first price. Use the first and last shared observed UTC dates within the selected range, so different trading calendars cannot create a mixed-date portfolio comparison. Share counts stay constant; this is price-only performance of today's holdings, not a reconstruction of historical trades or personal returns. Dividends and property are excluded. USD cash contributes to the starting denominator with zero price movement; interest is excluded.

As of 2026-09-17, both investment summary cards follow group selection and retain those figures in Cards/Table. The value uses compact currency (for example, $35,752.16 becomes $36k) and the selected group label. The current holding-value headline remains current. Every required market history within the selected group must be available; never show a partial aggregate. Unknown share counts (including manual-total investments), missing/one-point/non-overlapping history and provider failures withhold the change with context. Retry is available for failures within the selected group, including in Table view. Histories outside that group do not block its summary. No account snapshots, API, database or persistence changes are needed. Home's recorded-history chart is unchanged.

## Portfolio responsive actions — 2026-09-16

The updated Studio Display frame uses five asset columns; laptop/tablet/phone retain three/two/one with intrinsic text reflow. Phone Add asset and group filtering live in the page ellipsis, with all four periods always visible under Cards/Table. Automatic descending investment value order and market-performance calculations remain unchanged. Recurring retains Value/Name sorting in a compact disclosure; no new persistence or data behaviour.

## Historical Home market movement — 2026-09-22 (superseded by 0.2.16)

Supersedes the default recorded-history chart contract above. Net worth retains property equity and shows two decimal places at million scale (for example $1.23m); its exact saved/calculated dollar amount remains in the title and accessible label. The default chart is labelled Portfolio market movement, with signed percentage movement, dated percentage endpoints, a starting-value baseline and 1D/1W/1M/1Y/5Y ranges. 1M is the default. 5Y is the provider's maximum requested window, not a claim of complete coverage. 1D compares the latest two shared observed dates within seven calendar days, rather than an intraday feed.

Reuse Portfolio's market-price calculation: hold current share counts constant, sum each holding's observed price times its quantity (plus constant USD cash), and divide movement by the basket's starting value. This weights investments by their starting market values. It describes today's basket over the selected history; it is not the owner's realised or time-weighted historical return. Exclude property, dividends, deposits, withdrawals and past trades. Missing quantities/history/currency coverage withhold the whole chart; no partial basket, invented dates or interpolated prices. Cash-only shows zero movement without inventing a dated series.

Chart view offers Recorded value, defaulting that view to All, with the original dated investment snapshots and explicit saved-valuation/deposit/withdrawal scope. All-time value change retains its original current-value/first-snapshot calculation and is independent of chart range. Home and Portfolio share their existing account-scoped, in-memory market-history cache and three-request queue; changing ranges or moving between these surfaces reuses the data for five minutes. Retries fetch failed entries only, and account/route guards reject late UI updates. No new API key, external service, paid tier, database record or scheduled read is introduced.

Freshness investigation: the snapshot service reads saved holding quotes/manual valuations and does not fetch new market prices. Repeated saved inputs can yield flat history. The new Home chart reads the existing Yahoo daily-history bridge independently and displays actual common observation dates; existing snapshots are preserved. Source checks returned VFIAX through 21 September and BTC/USD through 22 September 2026. These read-only public checks do not establish production scheduler freshness or the owner's saved-quote state.
