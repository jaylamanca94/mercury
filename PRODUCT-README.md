...
- Portfolio is the full authenticated holdings workspace: it has independent search and sorting, every matching holding, quick add, Asset-detail navigation, and Cards/Table peer views backed by the same holding collection. Cards are the refresh default, Table provides a denser desktop/tablet scan and compact phone rows, and Property remains card-only. Its filters are All, Brokerage, Crypto, and Retirement; Retirement selects the holdings explicitly marked with that classification. Income is an authenticated planning workspace for saved recurring gross sources and live portfolio dividends, with Year/Month estimates, source management, search, and dividend sorting.
