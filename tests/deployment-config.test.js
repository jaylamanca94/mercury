...
test("Vercel applies a restrictive security envelope to every Mercury route", () => {
  ...
  assert.match(securityHeaders["content-security-policy"], /https:\/\/\*\.supabase\.co/);
  assert.match(securityHeaders["content-security-policy"], /https:\/\/cdn\.jsdelivr\.net/);
  ...
  assert.deepEqual(config.crons, [{ path: "/api/portfolio/snapshot", schedule: "0 * * * *" }]);
});
