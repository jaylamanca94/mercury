...
test("Home consumes Acadia without a Mercury presentation layer", () => {
  assert.match(styles, /^@import url\("acadia\.css\?v=20260914-c3547c1"\);/);
...
test("Plan is a separate Base-plan projection workspace with aligned portfolio charts", () => {
  ...
  assert.match(indexHtml, /<script type="module" src="brokerage\.js\?v=20260914-property-purchase-004"><\/script>/);
