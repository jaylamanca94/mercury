...
> **Composition update — 2026-09-04:** Home, Portfolio, Income and Plan are active. The approved summary → explanation → evidence → action composition below supersedes earlier chart-led Home and inline Income layouts. Legacy economy-dashboard guidance below is historical reference.
...
- Home uses 12 columns at 1280px: position/evidence span eight, monthly planning/review span four. Below this breakpoint the reading order is position, planning, review, evidence.
...
- Selected-range history stays compact below 30 distinct daily snapshots: “History building · N days recorded”, dates, latest recorded value and available movement. At 30 dates, show the existing range-controlled Acadia trend with timestamp-proportional spacing and a textual description. Duplicate dates never advance the gate. Never invent history or draw holding sparklines from account snapshots/raw quotes.
- Home and Portfolio share investment allocation: four largest valued holdings, deterministic ties, then Other investments. Dollar amounts and percentage shares use valued investments only, exclude property equity and disclose missing valuations. Zero value has an empty state. Portfolio search/filter/sort never changes this allocation.
- Home review groups at most four factual items in this order: unavailable setup/data, missing valuation/metric coverage, limited history, negative prior-close movement. Include relevant counts and destinations without inferring urgency or advice; show No review items when none apply.
- Top assets ranks holdings and property equity together in four rows. Preserve investment-detail navigation, property editing, classification and valuation semantics.
