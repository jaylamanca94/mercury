...
> **Home simplification — 2026-09-05:** The original Figma dashboard hierarchy is restored on Home: headline/history, compact allocation, three metrics and four asset cards. The Portfolio, Income and Plan contracts below remain active. Legacy economy-dashboard guidance is historical reference.
...
- Home uses only existing Acadia classes: Home Dashboard, Dashboard Layout, Dashboard Trend, Chart List/Progress, Dashboard Metric Grid and Asset Preview Card. Use the existing responsive Device Grid for asset cards (four on wide desktop, two on tablet, one on phones). Do not add Home overrides to `styles.css`. No visible Home title, monthly planning breakdown or review feed.
...
- Preserve the history card footprint below 30 distinct daily snapshots with one “History building · N of 30 days” message and available range movement. At 30 dates, show the existing Acadia trend with timestamp-proportional spacing and an accessible dated description. Duplicate dates never advance the gate. Do not invent history or benchmark series.
...
- Home’s three summary cards show prior-close Portfolio change, Expected annual growth from saved return assumptions, and Passive income as estimated annual dividends. Missing valuations make dependent totals unavailable; partial return/yield coverage must never be presented as a complete estimate. Loading dividends have a short explicit state.
- Top assets ranks holdings and property equity together in up to four compact cards. Each card shows identity/value, classification and a short valuation detail. Enter/Space opens investment details or the existing property editor. Add asset reuses Portfolio’s quick-add dialog; financial persistence and calculation rules remain shared.
- Portfolio leads with investment value and a primary Add asset action. Keep property equity and the weekly equivalent of every saved recurring investment beside their own sections, avoiding duplicate summaries. Investments comes first; portfolio-wide allocation is an initially collapsed native Acadia Accordion after the records. Its amounts, shares and valuation-coverage explanation remain unfiltered.
- Use Acadia Asset Preview Cards and Device Grid, matching Home’s identity/value/classification anatomy. Keep the price/share or manual-valuation line; detailed Return, Yield and Updated comparisons stay in Table and asset detail. The grid adapts to available space (verified at 1920/1440/1024/768/390/320px). Preserve the existing responsive table/object-row layout adapter.
...
- Recurring schedules retain saved cadence in compact Acadia rows with one Edit action. Property remains card-only with Market value, Mortgage balance and Equity. Its add/edit and deletion dialogs use Acadia’s compact form variant for phone containment. Vendor the canonical accordion plus/minus SVGs with the stylesheet so disclosures retain their affordance.
...
- Income's unenclosed summary leads with Planned balance, followed by expected income, earned/other recurring income, estimated dividends, planned spending and planned investing. Dividends stay explicitly annual regardless of summary period. Source rows show original amount/cadence, selected-period equivalent and Edit. Budget uses the full width for canonical monthly category amount, share of all planned spending and actions.
- Source/category editing uses existing dialogs with Save/Cancel and deletion confirmations; no inline autosave. Restore focus after a save or cancel, and return to Add if the invoking record was deleted. Keep separate dividend, source and category searches adjacent to their records; filtering affects counts/rows, never totals or percentages.
