...
- Portfolio uses an unenclosed summary of all investment value, property equity and the weekly equivalent of every saved recurring investment. Its attached Investments toolbar combines search, filters, count, sort and Page Header peer-view Tabs. Cards and Table share record state; revisiting Portfolio resets to Cards. Cards use three columns from 1440px, two from 768px and one on phones, leading with value above Price, Shares, Return and Yield. Table is investment-only with sortable headers and compact labelled phone rows.
- Recurring schedules retain saved cadence in compact rows. Property remains card-only with Market value, Mortgage balance and Equity.
