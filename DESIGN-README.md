...
- Two Acadia content cards show current **all-investment** value and selected-period **market-price change at current share counts**. Both percentage captions use the same aggregate rate. Sum current shares × per-unit price change between the first/last shared observed UTC dates; divide by the same basket's starting value, including constant USD cash. Screen-reader context excludes deposits, withdrawals, dividends and property and names actual dates; exact amount/date/source remain in the figure tooltip. Neither summary changes with asset group filtering or Cards/Table. The selected group's precise value/count/share remain in the section heading when scoped.
- Keep zero, positive and negative movements distinct. Missing share counts or any required market history withhold the total; one observation or fewer than two shared dates cannot invent movement. Loading/retry remain visible independently of the record filter and Cards/Table. Current holding value remains distinct from the dated market-price basket. This supersedes the earlier same-day recorded-snapshot summary choice; account snapshots no longer enter these cards.
