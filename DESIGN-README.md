...
Changing the period updates the graph and both change figures together. The balance and current chart endpoint must agree. Use true dated observations and straight segments, without invented historical holdings or smoothed intermediate movements. A missing range baseline names the actual start date instead of claiming a complete YTD/year return. A zero baseline omits percent; one point shows a dot and waits for another recorded day. Unknown valuations are unavailable, never zero. Source dates are New York dates, not request timestamps. Home no longer fetches reconstructed price-basket history.

Reuse unchanged Acadia 0.4.10 stack, field, trend, tabs, disclosure, typography and status tokens. Composition adapters only: wrapping change row; bounded responsive balance type with wrapping at enlarged text; existing chart heights and group grids. Arrow/Home/End keys keep roving-tab selection and focus together. The graph has a textual value/date summary. Entering Portfolio carries 1Y when selected and explicitly names its different price-change measure; other Home periods retain the last Portfolio range. Existing Cards/Table return behaviour is unchanged.

Evidence and limitations: [Home balance validation](automation/home-balance/2026-09-23/review.md). Earlier dated Home entries describe release history, not the current contract.
...
Figma section `110:10301`, with Studio Display `227:9805`, desktop `110:7662`, tablet `226:8198` and phone `226:8989`, supersedes the older Portfolio toolbar and summary composition below. Preserve the later asset-card, exposure-label and whole-percentage allocation corrections.

- Lead with Portfolio/Add asset. Investment assets automatically sort by current holding value, highest first, in both Cards and Table; unknown valuations appear last. Omit investment search, sorting menus and sortable table buttons. Investments/count pair with an All/Brokerage/Retirement/Crypto disclosure, shared period controls and Cards/Table. Desktop periods are separate 32px Acadia filter pills; tablets use a native disclosure. Phones place Cards/Table then all four period pills below the heading, and move Add asset/group selection into the page ellipsis. Both filter presentations share selection. Dialog close, group recovery and missing-card return focus target the visible phone menu. Touch controls keep Acadia's larger targets; menus and tabs reflow at 320px with 200% text. Bound and scroll the phone page-menu panel above the bottom dock.
- Two Acadia content cards show current **selected-group investment** value and selected-period **market-price change at current share counts**. Both percentage captions use the same aggregate rate. Sum current shares × per-unit price change between the first/last shared observed UTC dates; divide by the same basket's starting value, including constant USD cash. Screen-reader context excludes deposits, withdrawals, dividends and property and names actual dates; exact amount/date/source remain in the figure tooltip. Both summaries follow the selected asset group; Cards/Table keeps the same figures. The value card uses a compact headline and the selected group label (All investments when unfiltered). The selected group's precise value/count/share remain in the section heading when scoped.
- Keep zero, positive and negative movements distinct. Missing share counts or any required market history withhold the total; one observation or fewer than two shared dates cannot invent movement. Loading/retry reflect only histories required by the selected group and remain available in Cards/Table. Current holding value remains distinct from the dated market-price basket. This supersedes the earlier same-day recorded-snapshot summary choice; account snapshots no longer enter these cards.
- Investment cards retain 32px padding, 24px gaps, 64px rings and 160px charts; layout uses an intrinsic 20rem minimum: five columns at 2048px, three at 1512px, two at 834px and one on phones. Reserve empty columns so filtering does not enlarge a lone card. Apply Acadia’s wide shell token on the active Portfolio body so light and dark themes share the same width. Rings use total net worth, asset movements remain per-unit market prices, and the two scopes remain available to assistive technology. The founder removed the two persistent explanatory lines on 2026-09-16; no empty caption row remains.
- Recurring has three summary columns on desktop/tablet and one on phone. Its Value/Name sort uses the same native Acadia disclosure as Property; phone sort precedes Add recurring. The reference’s illustrative New label does not introduce a new sorting rule. Each schedule's native ellipsis menu opens the existing Contribution editor; Back restores focus to its disclosed Edit action. Saved weekly/monthly schedules still use 52/12 equivalents. The founder removed the visible equivalent-totals caption on 2026-09-16; retain it only as the summary's screen-reader description. Native action targets can make rows taller than the illustrative 27px text row.
...
- Keep Investments, selected value and count inline, with 1W/1M/6M/1Y and Cards/Table controls. Retain group selection and apply the automatic value ordering described above. Use wrapping Page Header Actions so tablet controls cannot squeeze the headline into a narrow column.
...
- Give holdings the full content width using the existing Grid and adaptive Insight Grid. Reuse Page Header Section for wrapping group/view/period controls. No stylesheet, token or shared-asset changes.
- Evidence and delivery receipt: `automation/portfolio/2026-09-14/review.md`. The responsive browser evidence uses synthetic holdings; it does not represent authenticated owner-data or physical-device acceptance.
...
- Home composes unchanged Acadia Card Trend, Content Card, Field, Grid, Insight Grid, Page Header, native Menu and Icon with Text. The September 14 Home group contract below supersedes the earlier no-adapter/top-assets composition. No visible Home title, monthly planning breakdown or review feed.
...
- Show all available history immediately: a single Acadia trend point for one recorded date, a line from two dates, and a short empty message with no countdown before the first record. Reuse Card Trend, Trend Axis, Field, Grid and the shared page-header tools. Set the supported trend-height token to the Home dimensions below; let the content card size naturally. Home history tabs use Acadia’s standard 32px desktop visual height and 4px rail inset, with its shared 44px target on phones and coarse pointers. Do not force the touch height on desktop. Keep the date/value endpoints visible, with timestamp-proportional spacing and an accessible dated description. Use the unchanged published Acadia `buildCardTrendPath` utility for gentle curves and matching area fills; its bounded tangents preserve observations without overshoot. The single-record point remains unchanged. Use the latest record for duplicate dates. The history selector sits beside net worth on larger screens and wraps on phones. Put one short investment/deposit scope note below the chart. Do not invent history or benchmark series.
- Portfolio allocation uses four largest valued holdings, deterministic ties, then Other investments, with Acadia progress bars and amounts. Home links into Portfolio through its group cards. Shares exclude property equity and disclose missing valuations. Zero value has an empty state. Filtering never changes this allocation. The expandable investment-allocation panel retains Acadia progress bars. Investment-card rings instead show each holding’s share of total net worth, including property equity; see the card contract below.
- Home follows Figma section `98:5696`, frames `194:1955`, `218:25404`, `218:26779`: 32px net-worth headline, unboxed chart, four summary cards, then Brokerage/Crypto/Retirement/Property. Keep 24px gaps and 32px standard card padding, bounded at enlarged phone sizes. Summary/group minimum heights are 117/209px and may grow for unavailable text. Four columns become two below 1200px and one below 768px. Phone content uses 24px reference insets at 400px; tablet navigation is centred. Acadia owns typography, colours, surfaces, icons and focus; `styles.css` owns only this composition.
- Home's chart-height token is 244px on desktop/tablet and 169px on phone. Preserve actual date/value endpoints, investment-only scope, daily ranges and All, accessible history descriptions and one-point/empty states. The reference's benchmark, net-worth delta, native OS status bar and tablet placeholder route names are not implemented as invented data or navigation. Existing real routes and safe-area dock remain. Accurate labels are Expected annual growth and Annual dividends. Routine estimate explanations are accessible descriptions/tooltips; incomplete/loading explanations remain visible. Day and All-time amounts retain their sign; percentage captions use the September 15 typography contract below.
- Portfolio group values use the same exclusive classifications and complete-valuation rules as Portfolio; retirement takes precedence over crypto. The header counts actual assets, not the four groups. Historical growth and dividend yield aggregate the existing rounded row estimates and withhold missing/pending coverage. Crypto omits the dividend icon; Property shows current equity with separately labelled market-value-weighted appreciation and its source. Cards and native menus open the relevant Portfolio destination, moving focus to its heading or property retry. Empty accounts keep Add asset; individual holding/property management stays in Portfolio. Unchanged background renders preserve a focused card/open menu.
...
- Portfolio uses compact interactive Acadia Content Cards in Grid plus the shared adaptive Insight Grid (four columns at 1440px, two at 768px and one on phones; enlarged text also reduces columns): map the supported content-padding token to Acadia dense padding, compose identity/value with Field and Object Card Header, and use a muted small classification/share or manual-valuation caption. Keep exact currency in the accessible link name and value tooltip. Omit the repeated classification inside a selected group. Price, Return, Yield and Updated remain in Table and asset detail. Use canonical Acadia Table Responsive composition: the table becomes labelled Object Cards when its container is 32rem or narrower. The horizontal table region is keyboard-focusable with visible focus and scroll guidance.
- Investment groups use canonical Acadia Page Header filter Badge buttons with native pressed semantics at every width. Wrap naturally; preserve the same controls and keyboard focus across viewport changes. A check icon identifies the selected group without relying on colour. The selected heading, precise value, count and portfolio share stay together; group values are available by selecting their filter, rather than repeated in a sidebar. Brokerage, Retirement and Crypto remain mutually exclusive, with Retirement taking precedence over Crypto. Withhold affected totals and all percentages when any valuation is missing; keep complete group values and explain unavailable allocation beside the headline. No percentage appears for zero totals. Cards and Table preserve group selection and automatic descending value order; revisits reset to Cards. Plain table headers retain aria-sort="descending" on Value to describe the fixed order. Empty groups offer View all investments, which restores focus to the group picker. Use an accessible name directly on the blank Actions table heading so an absolutely positioned hidden label cannot extend the document beyond its scroll region.
...
- Asset detail leads with Back, asset identity and a full-width market-price card, followed by your holding summary, investment profile and the primary Details form. The market card composes Acadia Content Card, Field, Page Header Section, native pressed range buttons, Card Trend and Trend Axis. Use the published bounded curve utility with timestamp-proportional positions, a first-price baseline and semantic positive/negative/neutral line colours; Up/Down/No change text conveys direction independently of colour. The prominent value is the latest daily market price in the chosen range, never shares multiplied by price or personal gain/loss. Show actual dated price endpoints and source, disclose USD per share/unit and dividend/personal-return exclusion. One observation produces a point without movement; empty/unsupported states produce no line. Keep 44px range targets, native keyboard activation, retry within the card and drafts intact during background updates. Use the same supported dense-padding and responsive trend-height tokens as Home; no local CSS. Its advanced fields live in Acadia Accordion disclosure; do not create a Mercury-specific details panel.
...
## Plan responsive reference — 2026-09-16

Figma Mercury section `129:5989` supersedes the earlier Plan composition: Studio `231:15610`, laptop `129:4939`, tablet `231:16236`, phone `231:17019`. Visible Plan title and age/date precede compact 1Y/5Y/10Y/20Y pills, defaulting to 1Y. Four summary cards sit above the unboxed 244px graph, followed by four compact current asset groups and Edit Details. Weekly Spending and Retirement use separate cards. Desktop and tablet retain four summary/group columns when text fits; phone stacks cards and fields. Intrinsic sizing permits enlarged text to reflow. Acadia content cards use bounded 32px insets with tighter tablet group insets to keep menus clear of headings.

Acadia owns surfaces, typography, semantic colours, controls, focus, dialogs and menus. Main change/growth amounts use theme-aware red/green; regular-weight smaller captions stay muted. The net-worth percentage uses brand teal for positive movement. Group ellipsis menus retain View navigation plus source/rate context. Group values are current; summary cards and graph are selected-year projections. Property shows equity. Keep accurate Net worth, Investment income / year, Expected annual growth and Income labels because the model includes property and non-employment income.

Native age selects preserve blank milestones and the existing 0–120 model bounds, with years-left hints. Income retains its amount and cadence control. Edits preview live with explicit Save Plan/Cancel, source hints and revision-aware conflict recovery; date of birth remains in Plan settings, also available through the phone heading menu. The year slider and exact investment/property breakdown remain inside Projection assumptions. Missing-data and depletion feedback stay visible when relevant. Source amounts and projection arithmetic are unchanged. Graph geometry comes from real monthly points through Acadia's unchanged curve helper; never reproduce illustrative market wiggles with invented data.

Plan settings uses Acadia's native date input for DOB, browser-local date presentation, bounds, written-date/current-age confirmation and existing save/conflict/draft protection. Birthdays use calendar dates. Synthetic responsive and interaction evidence is recorded in `automation/plan-responsive-frames/2026-09-16/review.md`; this is separate from authenticated owner-data and physical-device acceptance.


## Property appreciation and combined Plan — 2026-09-14

Use the existing Acadia form-modal, field grid, native selects and number input. Show City / locality, State and County / independent city as separate fields; state changes clear the county. Present annual appreciation with its county, ten-year period and FHFA release date, followed by an optional custom rate. Retain legacy location as helper text until confirmed, rather than parsing a guess. Portfolio keeps market value, purchase gain, appreciation, equity and debt distinct. Plan labels its combined chart and first statistic Net worth and gives an exact investment/property-equity breakdown. Missing growth and investment depletion have separate plain-language notices. Static annual source data refreshes only with a reviewed snapshot release. Shared Acadia assets remain unchanged.
...
## Home summary typography — 2026-09-15

The four Home summary cards use Acadia Text Red/Green for the main 20px bold currency value. Percentage captions are 16px regular-weight Acadia Text Medium, with no parentheses or positive plus sign; negative percentages keep their minus sign. Currency signs, arithmetic and missing-data behaviour remain unchanged. Examples: `-$5.4k -0.68%`, `+$32k 4.2%`.

This supersedes the September 14 rule that coloured both parts and enclosed absolute percentages in parentheses. Home-scoped fallback mappings mirror Acadia `55026a0` Text aliases until the next full vendor adoption: dark red-50/green-40/cool-gray-40; light red-60/green-50/cool-gray-70. Native aliases take precedence when available; other routes and shared vendor assets remain unchanged.


## Portfolio asset cards — 2026-09-15

- Reference: Figma Mercury `222:3187`, confirmed with the founder’s positive/negative card screenshot. Adopt Acadia content cards and Card Trend without editing vendored assets. Preserve the adaptive grid, 32px bounded card insets, 24px section gaps, a 64px allocation ring and a 160px trend viewport. Match shared light/dark/system surfaces, focus and touch sizing. The period rail uses canonical 32px desktop tabs and 44px phone/coarse-pointer targets.
- Cards lead with symbol, current holding value and asset exposure/type; the upper-right ring represents current holding value divided by the same complete net worth disclosed on Home, including property equity. Group filters, Cards/Table and market period do not change that denominator. This differs deliberately from the expandable investment-only allocation panel and selected-group investment share.
- One shared 1W/1M/6M/1Y selector (default 1W) controls card market histories. Footer order is period, signed percentage, signed USD change **per share/unit**, never a gain based on the owner’s share count or purchase cost. Use the first and last observed daily prices within the selected calendar range; accessible text includes provider, actual dates and the exclusion of dividends/personal gain. Normal dollar changes show cents; sub-cent changes retain up to eight decimal places. Screen-reader context: “Price change per share/unit · Allocation of total net worth.” The founder removed its visible caption on 2026-09-16.
- Positive trends and percentages use Acadia brand teal, negatives use its theme-aware red Text role, and flat changes stay neutral. The dollar caption is regular-weight muted text. Use actual observations and the unchanged shared curve builder with a starting-price baseline. A single observation shows a point and unavailable change; no history shows a quiet empty state and dashes. Retain loading, retry, account isolation, concurrency limits and focus recovery.
- Reuse classification: bounded product composition adapter for data-driven rings; Acadia currently has no published ring primitive. A CSS conic gradient renders the exact share. Its visible label rounds to a whole percentage (23.67% displays as 24%); the accessible label and hover title retain the more precise percentage. No Figma example arc or numeric fixture is used as real data. Missing valuations/property reads or non-positive net worth withhold the share. A holding above 100% of net worth shows its actual percentage and negative-property-equity explanation without a ring. Forced colours retain the textual value. Candidate for later graduation to Acadia; no Acadia library publication is part of this change.
- Cards remain keyboard/click destinations for existing asset details. Return/yield badges, classification badges and quick-action menus are removed from cards; editing and additional metrics remain in asset details and the existing Table view. Property cards are unchanged.
- Local synthetic-data verification is recorded in `automation/portfolio-cards/2026-09-15/`. Production and physical-device acceptance are separate from automated/browser checks.


## Portfolio asset-type subtitles — 2026-09-16

The second line of each investment card identifies exposure/type, rather than the saved name or generic “Other” instrument fallback. `holdingAssetTypeLabel` resolves exact, case-normalised fund symbols from the issuer-verified reference below, then a meaningful saved allocation category, then a known instrument type. Explicit cash/crypto instrument types retain their labels. Unknown metadata displays “Unclassified”; names, ticker substrings and retirement status are not used to guess exposure. Names and instrument fields remain unchanged in storage and other views. This is a display-only correction with no provider request, migration or account-data write.

| Symbols | Card subtitle | Issuer reference |
| --- | --- | --- |
| VFIAX, VOO | S&P 500 | [Vanguard mutual-fund/ETF equivalents](https://www.vanguard.com/pdf/etfpdf.pdf) |
| VSMAX, VB | Small-Cap | [VSMAX](https://advisors.vanguard.com/investments/products/vsmax/vanguard-small-cap-index-fund-admiral-shares), [VB](https://advisors.vanguard.com/investments/products/vb/vanguard-small-cap-etf) |
| VTIAX, VXUS | International | [VTIAX](https://advisors.vanguard.com/investments/products/vtiax/vanguard-total-international-stock-index-fund-admiral-shares), [VXUS](https://advisors.vanguard.com/investments/products/vxus/vanguard-total-international-stock-etf?compositionTabBox=1) |
| VBTLX | Bonds | [Vanguard fund fact sheet](https://workplace.vanguard.com/assets/corp/fund_communications/pdf_publish/us-products/fact-sheet/F0584.pdf) |
| VGT | Technology | [Vanguard fund profile](https://advisors.vanguard.com/investments/products/vgt/vanguard-information-technology-etf?mkwid=HU7FLMVq) |
...
## Historical Home market movement — 2026-09-22 (superseded by 0.2.16)

Home keeps its existing net-worth/range row, unboxed curved chart, four summary cards and four group cards. Only the net-worth headline gains two decimal places at million scale. A second compact row separates the chart's Portfolio market movement label and signed period percentage from net worth, with Acadia's native Chart view select offering Market / Recorded. The market range defaults to 1M; its longest option is 5Y, while recorded history retains All. The original roving-tab keyboard pattern and dated endpoints remain.

Use Acadia's unchanged curve utility, trend area/baseline/line, controls, field, cluster and page-header composition. The market line starts at 0%; percentage endpoints and source/scope text show real common dates. Signed text accompanies direction colour. State explicitly that this is based on current holdings and price change only, excluding dividends/property. The precise accessible summary/title describes constant current shares and starting-value weighting, and avoids claiming personal historical return. All-time value change is visibly distinguished from market movement.

Product CSS only wraps the chart heading/select at constrained widths and applies existing semantic colour tokens. Loading, empty/cash-only, missing shares/history and retryable provider failure remain visible; never draw a partial portfolio or carry a previous successful line into an unavailable range. Keep retry focus during loading, then move it to the recovered percentage. Evidence: `automation/home-market/2026-09-22/review.md`. Acadia vendor assets are unchanged.
