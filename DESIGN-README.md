...
- Reuse classification: bounded product composition adapter for data-driven rings; Acadia currently has no published ring primitive. A CSS conic gradient renders the exact share, with a percentage and accessible label. No Figma example arc or numeric fixture is used as real data. Missing valuations/property reads or non-positive net worth withhold the share. A holding above 100% of net worth shows its actual percentage and negative-property-equity explanation without a ring. Forced colours retain the textual value. Candidate for later graduation to Acadia; no Acadia library publication is part of this change.
