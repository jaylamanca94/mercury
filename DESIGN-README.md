...
> **Home simplification — 2026-09-05:** The original Figma dashboard hierarchy is restored on Home: headline/history, compact allocation, three grouped metrics and four asset cards. The Portfolio, Income and Plan contracts below remain active. Legacy economy-dashboard guidance is historical reference.
...
- Net worth combines current investments and property equity. Missing valuation or property coverage makes the headline unavailable. Recorded snapshot differences are labelled Portfolio value change, exclude property equity and may include contributions/withdrawals; they are not investment returns.
- Use a compact content card below 30 distinct daily snapshots with one “History building · N of 30 days” message and available range movement. At 30 dates, expand to the existing Acadia Dashboard Trend with timestamp-proportional spacing and an accessible dated description. Duplicate dates never advance the gate. Do not invent history or benchmark series.
- Home and Portfolio share investment allocation arithmetic: four largest valued holdings, deterministic ties, then Other investments. Home shows compact Acadia progress bars and percentage shares; Portfolio retains amounts. Shares exclude property equity and disclose missing valuations. Zero value has an empty state. Filtering never changes this allocation. Use existing progress bars because Acadia has no donut primitive.
- Home groups its three read-only metrics inside one content card beneath net worth: prior-close Portfolio change, Expected annual growth from saved return assumptions, and Annual dividends as an estimate. Rule Grid adapts the metrics to their available width; allocation sits alongside the overview on larger screens. The headline uses Acadia Title with Read Only anatomy, and the range header wraps using Cluster. Missing valuations make dependent totals unavailable and receive a visible explanation beside net worth; partial return/yield coverage must never be presented as a complete estimate. Loading dividends have a short explicit state.
- Top assets ranks holdings and property equity together in up to four compact cards. Each card shows identity/value, classification and a short valuation detail. Enter/Space opens investment details or the existing property editor. View all opens Portfolio. Add asset reuses Portfolio’s quick-add dialog; financial persistence and calculation rules remain shared.
- Portfolio leads with an Acadia Title investment value and a primary Add asset action. Keep property equity and the weekly equivalent of every saved recurring investment beside their own sections, avoiding duplicate summaries. Investments comes first; portfolio-wide allocation is an initially collapsed native Acadia Accordion after the records. Its amounts, shares and valuation-coverage explanation remain unfiltered.
- Portfolio composes Acadia Asset Preview Cards with identity/classification in the header, a neutral Card Metric Value in the content, and the price/share or manual-valuation line below. Use the shared Grid with two columns alongside the sidebar, three on wide desktops (1600px and above), and one on phones; Home retains its compact Device Grid. Detailed Return, Yield and Updated comparisons stay in Table and asset detail. Preserve the existing responsive table/object-row layout adapter. Verified from 320px through 2560px.
- Investment groups form a 256px Acadia Side Nav beside the holdings and allocation on desktop. Each selectable Read Only row shows its exact value, asset count and share using Acadia Progress. Below 1024px, use one native group selector with values in its options and selected-group count/share above the holdings. The sidebar replaces the former classification dropdown; search and native sort remain attached to the records. All investments, Brokerage, Retirement and Crypto are mutually exclusive groups: Retirement takes precedence, Crypto contains only non-retirement crypto, and Brokerage contains the remaining holdings. Selecting a group updates the section heading and both Cards/Table views; group totals and shares remain unfiltered. Withhold affected group totals and all percentages when any valuation is missing, while retaining complete group values and editable unvalued assets. No percentage is shown for a zero total. Exact group amounts reconcile; displayed percentages are rounded to one decimal. Keep the sidebar controls stable across rendering to preserve keyboard focus. The narrow Mercury layout adapter owns the sidebar/content split and responsive visibility; Acadia owns the controls, surfaces and type. Sort remains available in both views at every width. Cards and Table preserve search/filter/sort, and revisiting Portfolio resets to Cards. No matches offers Clear filters and returns focus to search; counts announce changes without making the whole workspace a live region.
...
- Income uses Acadia Title for the unboxed Planned balance, followed by one Content Card with a three-column Grid for Expected income, Planned spending and Planned investing. Earned/other income and estimated dividends form a quiet breakdown beneath Expected income. The grid stacks on phones. Overview places Sources alongside Dividends on tablet/desktop, with Sources first on phones. Source Content Cards show selected-period income, saved amount/cadence and wrapping Edit/Delete actions. Dividends use one Content Card with compact Object List rows; keep annual income and yield explicitly labelled regardless of planning period, and avoid repeating an asset name as its subtitle. Budget retains its existing responsive table adapter for monthly category amounts, share of all planned spending and actions. Do not introduce local Income layout or typography rules when these Acadia compositions suffice.
...
- Persisted history uses Acadia SVG trends with textual summaries and the 30-distinct-date gate above; never simulate history to fill space.
...
## Add asset intrinsic sizing — 2026-09-07

- Add asset must hug its visible content and remain vertically centred. Safari stretches Acadia's grid dialog with opposing block insets despite `height: fit-content`; Mercury's narrow `#asset-dialog` compatibility adapter uses auto height, a 50% block-start/auto block-end anchor and a -50% vertical translation. Preserve Acadia's width, padding, controls, grid spacing, max-height and internal overflow.
- Verified in native Safari at 2560 × 1388: 560 × 428px collapsed and 560 × 528px expanded. Desktop-browser 320px preview: 288 × 498px initially, capped at 708px in a 740px viewport with tall manual recovery; Cancel/Add remain reachable by internal scrolling. Closed native dialogs remain hidden. No controller or financial behaviour changes.
