...
- Home uses only existing Acadia classes: Card Trend, Chart List/Progress, Content Card, Field, Grid, Insight Grid, Divider and Page Header. Combine Grid with Insight Grid for content-aware wrapping; asset cards use two columns when their own available width permits and one otherwise. Do not add Home overrides to `styles.css`. No visible Home title, monthly planning breakdown or review feed.
...
- Home follows the September 14 layout reference: net worth and range controls sit over one unboxed full-width chart, followed by four separate dense Content Cards in Day change, All-time change, Expected annual growth and Annual dividends order. Top assets spans the next full-width section with four adaptive cards; Allocation stays in Portfolio. Use Acadia Field for compact dated endpoints, the published curve utility and `clamp(10rem, 16vw, 12rem)` for the chart-height token. Summary values lead their labels with source/coverage hints beneath. Growth and dividend percentages use current investment value as the denominator and remain hidden when coverage is incomplete or the denominator is zero. The adaptive Insight Grid yields four desktop, two tablet and one phone column. Top assets keeps identity/value together, names or explicit property-equity classification beneath, and a separate unit-price/share or purchase-price line. Use precise accessible values and preserve keyboard navigation, Home return focus, Add asset and property-edit actions. The reference is a layout target; never invent its illustrative numbers, net-worth percentage, benchmark, or intraday series. Daily 1D/1W/1M/1Y controls and All filter real snapshots, with one point before enough data exists for a line; totals and day/all-time changes stay independent of range. No local CSS or shared asset edits.
