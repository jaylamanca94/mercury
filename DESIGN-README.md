...
- Figma node `82:1863` establishes Home's chart-led hierarchy, refined into one compact portfolio-history card, one shallow metric strip, and one ranked Top assets list. The card visibly labels current `Net worth` separately from `Portfolio change · {period}` so property equity is never mistaken for investment performance.
- The September 4 Home refinement gives net worth the Acadia display size and an explicit `Investments + property equity` basis. Selected-period performance and its existing range tabs sit opposite it on desktop and below it on phones. Chart rows have a bounded height while the card grows naturally with labels, avoiding SVG intrinsic-size expansion and clipped history context.
- Keep Home counts neutral and secondary. Rank, identity, supporting detail, value and chevron form the desktop asset scan; phones retain details beneath the identity and value. Mobile section headings precede their actions, estimates form compact label/value rows, and an empty asset list has no residual panel border.
- Consume canonical `acadia.css` and its font assets unchanged. Keep `styles.css` limited to narrow Mercury compositions—such as the Home chart context, metric strip, and ranked rows—without introducing local tokens or competing component foundations.
...
- Performance renders only persisted daily snapshots. It uses the Acadia Card Trend presentation with a textual alternative, chronological `3M · 6M · 1Y · All` tabs, the selected range's start and end, its last plotted snapshot as `Latest portfolio value`, and the earliest authentic date as `History available since…`. The chart remains empty until the range contains two stored points. Do not add Figma's illustrative S&P 500 series until Mercury stores or retrieves a date-aligned benchmark.
- The shallow Home metric strip leads with `Since prior close` to distinguish daily portfolio movement from selected-period chart movement, then presents neutral Estimated annual growth and Estimated annual income with their assumptions and trailing-yield bases. Both daily movement values share the same positive, negative, zero, or unavailable tone.
- Top assets ranks saved holdings and property equity together and shows up to four compact rows with a `N of N shown` caption. Holding rows name Brokerage, Retirement, or Crypto before any specific instrument type; property rows disclose equity, market value, and mortgage balance. Every row retains direct navigation or editing, while `View portfolio` replaces Home creation actions.
- Portfolio begins with a shallow Acadia muted summary of complete investment value, aggregate property equity, and the weekly equivalent of every saved recurring investment. Those figures never change with Investments search or filters; inaccessible property data is labelled `Not set`.
- Portfolio Investments keeps Add asset beside the section heading and groups `Search assets`, sort, classification filters, and Page Header peer-view Tabs in one attached Acadia Toolbar. Cards and Table remain presentations of the same searched, filtered, and sorted holdings. Cards are a concise two-column desktop view with visible Price, Shares, Return, Yield, and Value labels; Table stays the refresh-optional dense peer view, and phones use Acadia object rows rather than a horizontally scrolling financial table.
- Recurring schedules render as compact rows inside one muted panel, retaining each holding's canonical cadence while the section caption reports the annualised total divided by 52 as a weekly equivalent. Property remains card-only and labels current Market value, Mortgage balance, and derived Equity; Mercury does not invent purchase-price or since-purchase figures.
