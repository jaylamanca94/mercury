...
- Two Acadia content cards show current **all-investment** value and selected-period **market-price change at current share counts**. Both percentage captions use the same aggregate rate. Sum current shares × per-unit price change between the first/last shared observed UTC dates; divide by the same basket's starting value, including constant USD cash. Screen-reader context excludes deposits, withdrawals, dividends and property and names actual dates; exact amount/date/source remain in the figure tooltip. Neither summary changes with asset filtering, search, sort or Cards/Table. The selected group's precise value/count/share remain in the section heading when scoped.
...
- Give holdings the full content width using the existing Grid and adaptive Insight Grid. Reuse Page Header Search and Section for the wrapping search/view/sort controls. No stylesheet, token or shared-asset changes.
