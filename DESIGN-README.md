...
- Portfolio Investments uses Acadia's Page Header peer-view Tabs to switch between Cards and Table presentations of the same searched, filtered, and sorted holdings. Cards remain the refresh default; desktop and tablet use the compact native table, while phones use Acadia object rows rather than a horizontally scrolling financial table. Property remains card-only.
