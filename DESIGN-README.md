...
- Portfolio composes Acadia Asset Preview Cards with identity/classification in the header, a neutral Card Metric Value in the content, and the price/share or manual-valuation line below. Use the shared Grid with three columns from tablet upward and one on phones; Home retains its compact Device Grid. Detailed Return, Yield and Updated comparisons stay in Table and asset detail. Preserve the existing responsive table/object-row layout adapter. Verified from 320px through 2560px.
- Search, native All assets/Crypto/Retirement filtering and native sort are attached to Investments. Remove the redundant Brokerage filter. Sort remains available in both views at every width. Cards and Table preserve search/filter/sort, and revisiting Portfolio resets to Cards. No matches offers Clear filters and returns focus to search; counts announce changes without making the whole workspace a live region.
...
- Plan uses a compact Acadia Read Only header with one primary Edit assumptions action, followed by current investment value and annual recurring investments. A single Status Row identifies missing assumptions, loading settings or incomplete valuations; hide and clear both charts until the complete investment outlook is available. Review Portfolio leads to the existing valuation repair flow. Never project a partially valued portfolio as a complete total.
- The available Outlook pairs Portfolio outlook and Projected portfolio income in a shared two-column Acadia Grid, stacking on phones. Each Content Card leads with its future value, explicitly labels the selected horizon and retains the current value beneath its chart. Use existing Card Trend and range endpoint typography; horizon buttons retain native pressed semantics. Assumptions use one three-column read-only grid with a single contextual Edit action and explicit Plan override/From Portfolio source captions. Property equity is compact, separate context with Manage in Portfolio; it stays excluded from both projections. No Plan-specific layout or typography rules.
- Base plan editing focuses Expected annual return on entry. Blank rates inherit Portfolio calculations when coverage exists. Preserve explicit Save/Cancel, retained failed drafts, unsaved-discard confirmation, locked pending writes and return focus to the invoking control.
